...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                options=(
                                    ...
                                    [{'label' : value, 'value' : value} for value in spacex_df['Launch Site']]
                                ),
                                value='ALL',
...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        # Lọc dữ liệu cho site cụ thể
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
        # Đếm số lượng 0 và 1 để vẽ biểu đồ
        success_counts = filtered_df['class'].value_counts().reset_index()
        success_counts.columns = ['class', 'count']
        fig = px.pie(success_counts, values='count', names='class', 
                     title=f"Total Success Ratio for site {entered_site}")
        return fig
...
def get_scatter_plot(selected_site, selected_payload):
    # Lọc theo cân nặng trước (Slider trả về list [min, max])
